...
| T13 | P1 | The v2.10.0 measure cap left prose **left-anchored against a blank right half** (user feedback), and the drawer — now the menu on half-screen desktops — opened as a **full-viewport sheet** with 1250px-wide rows. | `components/blocks/html/_html.scss` · `_menu-drawer.scss` | ✅ done (v2.11.0) — the capped prose and the page-title block share a **centred article column** (verified: both 704px on the same left edge; wide data blocks stay full-bleed). Gotcha on record in `_html.scss`: page-grid layouts make blocks grid items, and auto inline margins shrink a grid item to fit-content — the title needs `width: min(100%, var(--measure-narrow))`, not `max-width`. The drawer is now a **right-anchored panel** (`min(24rem, 100%)`, hairline + `--shadow-xl`, `--z-drawer`, slides from under the hamburger; full-bleed sheet kept below `$sm`). Desktop inline menu inspected and left alone — pills, tinted active state and the branded mega-panel are already on-system. |
