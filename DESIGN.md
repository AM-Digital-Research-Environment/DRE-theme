...
- `partner-{ujkz,ulg,moi,rhodes,ufba}.webp` — the African Multiple Research
  Centres, shown on consistent light plaques in the footer strip.
- `bayreuth.webp`, `ubt.webp`, `uni-bayreuth-africa-multiple-logo.webp` — available
  for footer / settings use.

The header logo is overridable via the **Logo** theme setting; the partner strip
is toggled by **Show partner universities**.
...
- **Footer** — deep forest band (`--footer-*`), partner-universities strip, inline
  `currentColor` social icons (so they show on the dark band), bottom bar.
